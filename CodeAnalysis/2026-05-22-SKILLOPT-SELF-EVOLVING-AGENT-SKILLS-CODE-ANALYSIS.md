...
> [!warning] 取得限制（誠實揭露）
> 這支影片**字幕已關閉**，音檔下載（YouTube SABR + SSL 憑證問題）與 NotebookLM 來源匯入在本次環境**皆未成功**，因此**未能取得 10 分鐘的完整逐字稿**。以下僅根據可靠取得的**影片標題與官方說明欄（description）**整理，**不含**影片口播的細節論述。

- **框架定位**：影片標題與說明欄把 SkillOpt 定位為「**把 prompt 當成可訓練的模型來訓練**、重新定義提示詞工程」——呼應本筆記「文字空間優化器 / 深度學習訓練隱喻」的主軸（craft → science）。
- **影片涵蓋主題**（依說明欄逐字）：為什麼要訓練 skill、怎麼訓練、結果有多猛、以及「**對你寫 prompt 的方式意味什麼**」。具體論述需看影片本身。
- **數據**：說明欄點名 **GPT-5.5 直接對話準確率 +23 分**（與本筆記 benchmark 表的 +23.5 一致）。

> [!note] 事實補充：作者單位
> 影片說明欄明確指出 SkillOpt 是 **Microsoft 與上海交通大學、復旦大學、同濟大學** 的合作（本筆記原僅標註 Microsoft，已於 frontmatter 補上；仍待 arXiv PDF 獨立核實——見 Open Questions）。

> [!info] 與本筆記的關係
> 影片屬 10 分鐘科普向；本筆記在機制、benchmark 細節、與 TextGrad/GEPA/DSPy 比較、成本/風險上都更深。**就「重點有沒有漏」而言：核心技術重點沒有漏；影片能補的是「作者單位」這個事實，以及一個適合大眾的科普框架。**（待日後取得逐字稿，可再回頭核對影片是否有額外觀點。）
