...
> [!success] 已取得完整逐字稿（使用者提供）
> 以下是與本筆記比對後，**影片真正補強、值得寫進來的洞察**（皆出自影片口播；數字與本筆記既有資料交叉核對）。

### 影片補強的 5 個洞察

1. **「能跑 ≠ 寫得好」才是手寫 skill 的真正盲點** — 影片用最白話的方式講動機：「skill 能跑不代表寫得好；同一任務 A 寫法 60 分、B 寫法 85 分，你跑過 A 沒爆就以為到頂了。」比本筆記「Why」一節更具體點出**手寫時代的隱性天花板**。
2. **驗證閘門的隱藏紅利＝可解釋性（explainability）** — 不只是「擋退步」：因為每次編輯都對齊驗證集，你能**看到哪一筆編輯讓哪幾題加分、哪幾題扣分**。手寫時代「永遠不知道自己錯在哪」，SkillOpt 讓每步改動都可歸因。（對應 `evaluation/gate.py`，本筆記原本沒點出這個好處。）
3. **optimizer／agent 分家的兩個理由本筆記沒講全** — (a) **避免自我合理化（self-rationalization）**：寫 skill 是 meta-cognition、跑任務是 execution，綁在同一模型會互相干擾——這正是 **generator／discriminator 分開** 的同一個 idea；(b) **成本洞察**：可用**便宜小模型當 optimizer、貴大模型當 agent**，且「連品質較差的小模型當 optimizer 都能優化得很好」。（這才是我先前誤刪後該補的方向——真正的發現是「**小模型當 optimizer 也行**」，不是「小模型追上大模型」。）
4. **學習率預算的反直覺實證細節** — 影片補上本筆記只有概念、缺的數字：**一次編輯 4–8 處最有效；最終最強 skill 常常只保留 1–4 個被接受的核心修改；學習率設太大反而卡在局部最優（local optimum）**。呼應本筆記 ablation 表的 `lr=4 default`，給出「小步走、慢疊代最強」的工程直覺。
5. **尺度不變性（Scale Invariance）的哲學含義** — 本筆記只列了遷移數字，影片給了**為什麼**：skill 可跨模型搬、不用 retrain，「**prompt 反而比 weight 更通用**」，因為 skill 學到的是**任務的本質結構**、而非某個模型的特異點（idiosyncrasy）。論文主張：**skill 應被當作 agent 的「外部凍結狀態（external frozen state）」來訓練**——這句是全篇論點核心。

> [!quote] 範式轉移（值得記住的一句）
> 「以後不會再有『只會寫 prompt 但不會建驗證集』的工程師——兩者會合而為一。**驗證集設計（validation set design）會變成新的核心技能**；會選題目、會建測驗的人，會是 AI 工程師裡最有價值的角色。」

> [!tip] 可帶走的營運洞察（企業／開源）
> **「訓練一次、到處部署」**：把 skill 訓練集中在算力最強的環境跑，再把成品分發到所有環境/模型，省下「每個環境各訓一遍」的成本；開源則是「一個 skill 訓練一次，所有模型受益」。對 [[2026-01-09-OH-MY-CLAUDECODE-MULTI-AGENT-ORCHESTRATION|多代理人系統]] 與 connsys-jarvis，這是讓 system prompt/skill **集中精煉、全隊共用** 的直接依據。

> [!note] 與既有資料的交叉核對
> - **數字**：直接對話 **+23.5**、Codex **+24.8**、CoT/Claude Code **+19.1**、**52/52 全勝**——其中 Codex/Claude Code 用的是**論文摘要版**數字（+24.8／+19.1），與本筆記「效能基準」表中專案頁的 +21.8／+18.6 不同；可見兩組數字確實並存（解答了本筆記原 Open Question 的一半）。
> - **成本**：影片說整個訓練「**幾小時到一天**，視驗證集大小／模型呼叫速度／預算而定；算力不便宜，但一個 skill 訓練一次可長期使用，性價比高」——部分回答了本筆記「訓練成本」的 Open Question（仍缺具體 token/金額）。
> - **術語對應**：影片把 rejected-edit buffer 直接叫 **Replay Buffer（編輯緩衝區，存失敗當反面教材）**、把 epoch 末的 slow/meta 更新類比 **meta-learning 的「盤點筆記」**——與本筆記機制一致，只是用更白話的 ML 詞彙。
> - **作者單位**：影片口播只提 **上海交大、復旦**；官方說明欄另列 **同濟**——本筆記 frontmatter 暫列三校，仍待 arXiv PDF 核實（見 Open Questions）。
