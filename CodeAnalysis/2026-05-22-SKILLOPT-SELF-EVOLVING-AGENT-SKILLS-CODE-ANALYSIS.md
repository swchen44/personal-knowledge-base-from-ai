...
> [!info] 為什麼補這段
> 透過 NotebookLM 對影片來源做忠實摘要後與本筆記比對。**結論：本篇的核心技術重點沒有漏**——影片是 10 分鐘科普向，在機制細節、benchmark 數字、與其他方法比較、成本/風險上都比本筆記淺。影片真正能補的是「科普框架」與一個事實（作者單位）。

1. **框架定位：prompt 從「手工藝（craft）」走向「科學（science）」** — 影片明講「手動調 prompt 的時代正在結束」，把 SkillOpt 視為提示詞工程從「憑直覺手寫」轉為「可量化、可複現、能自動迭代」的典範轉移。
2. **影片最強調的白話重點 = 驗證閘門帶來的「單調改善」** — 影片用白話點出本筆記的核心機制：優化器讀取成功/失敗結果來改技能文件，但**每次修改都要先通過驗證、只有變好才保留、否則退回**，所以技能會穩定地越變越強。（對應本筆記 `evaluation/gate.py`／`evaluate_gate` 一節。）
3. **對日常寫 prompt 的啟示（僅觀念層）** — 影片給的是觀念啟示：別再執著「一次手寫出完美 prompt」，而是**準備能評分的資料、建立可衡量好壞並自動迭代的流程**。（影片止於趨勢與觀念，**未**給「自己在家怎麼做」的逐步操作清單。）

> [!note] 事實補充：作者單位
> 影片明確指出 SkillOpt 是 **Microsoft 與上海交通大學、復旦大學、同濟大學** 的合作（本筆記原僅標註 Microsoft，已於 frontmatter 補上；另見 Open Questions——此單位仍待 arXiv PDF 獨立核實）。

> [!warning] 影片「沒有」涵蓋的（故不從它補充）
> 影片**未**展開 52/52、各 benchmark 細表、消融與遷移數字，**未**點名 TextGrad/GEPA/DSPy 做比較，也**未**談運算成本／token 消耗／對驗證集過擬合等風險——這些本筆記的「效能基準」「架構師觀點」「方案批判三問」已涵蓋。
