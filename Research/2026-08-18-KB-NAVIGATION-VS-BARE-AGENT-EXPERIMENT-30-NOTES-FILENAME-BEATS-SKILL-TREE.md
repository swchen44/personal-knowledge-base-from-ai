...
**（e）實驗 G 最終重測 12 題**（審查者提議的三層重分組：以「答案的可發現訊號住在哪一層」分類；A/B/C 三組各跑一次＝36 runs；實驗 H 以同一套題在 146 篇語料重跑 A/B＝24 runs）：

```mermaid
pie showData
    title 實驗 G／H 題組的三層型態分布（各層 4 題；G 每題 ×3 組、H 每題 ×2 組）
    "L1 檔名層（測最低成本路徑）" : 4
    "L2 索引層（測 D14 浮升假說）" : 4
    "L3 內容層（測全文 grep 兜底）" : 4
```
...
**設計**：題目不變（G 的 12 題），語料 30→**146 篇／4.2MB**（4.75×，即真實知識庫全庫，排除含 gold 答案的本論文筆記）；tree-B5 直接採用真實 KB 的 11 分類 INDEX（64K，**比手工版大 13 倍、摘要未經 D14 精煉**——這本身就是規模化的真實樣貌）。四項預測跑前寫死存檔（assets/predictions-H.md）。本輪僅跑 A/B 兩組（24 runs）；**C 組（Sonnet）未跑**——模型可換性已於實驗 G 證實（正確率同、+30% tokens），規模效應與模型選擇為獨立變因。

**事前預測計分**（詳細數據見 (a)(b)）：

| 預測 | 判定 |
|------|------|
| P1 正確率接近全對 | ✅ 24/24 零失誤 |
| P2 「A 呼叫最少」維持（審查者預測） | ✅ 三層全部維持；但「A 的 L3 升至 5–6 次」過度悲觀（實際 4.25） |
| P3 B 的 L3 回本擴大至 −20~30% | ❌ **被推翻反轉**——B 反虧 22% |
| P4 A 的成本升幅 > B | ❌ 相反——A 僅 ±7%，B 的 L3 +33% |

#### （a）146 篇結果（單獨分析）

| 層 | 【146 篇】A 裸奔<br/>正確｜呼叫｜千 tokens｜秒 | 【146 篇】B 導航<br/>正確｜呼叫｜千 tokens｜秒 |
|---|---|---|
| L1 檔名層 | 4/4｜**3.0**｜31.4｜19.4 | 4/4｜4.25｜33.6｜24.4 |
| L2 索引層 | 4/4｜6.0｜35.6｜45.2 | 4/4｜6.5｜36.7｜42.8 |
| L3 內容層 | 4/4｜**4.25**｜36.1｜34.6 | 4/4｜6.25｜**44.1**｜37.5 |

**146 篇單獨的四個 insight**：

1. **24/24 全對，零失誤**——146 篇／4.2MB 仍完全在「全文可掃」的正確率安全區內。
2. **A 組每一層呼叫數都最少**（3.0／6.0／4.25 vs B 的 4.25／6.5／6.25）——審查者的事前預測正確。A 組最貴的一層是 **L2 而非 L3**：L2 的口語題（比喻、狀態機）缺乏可 grep 的獨特詞，agent 要多輪試探同義詞（L2b 8 次、L2c 7 次呼叫）；L3 題反而因為關鍵詞獨特（體檢、CTF、三小時）grep 一擊即中。**「難不難找」取決於查詢有沒有獨特詞，而不是事實埋多深**。
3. **B 組在 146 篇的致命傷是 index 肥大**：11 個分類 INDEX 共 64K、單檔最大 30K+，B 組光「讀路由表」就吃掉可觀成本（L3a-B5 高達 56.6k tokens）——導航在每一層都輸給裸奔。
4. **多源交叉引用紅利**：大語料讓兩組自發引用 2–4 篇佐證（AutoDream 題 4 篇、規則條數題 3 篇、健檢題 2 篇），答案比 30 篇時代更豐富、更有互相印證的可信度——規模的正面效應落在**答案品質**而非檢索成本。

#### （b）146 篇 vs 30 篇對照（A、B、C 全展開）

**呼叫數對照**（C 組僅 30 篇有數據，列出供參照）：

| 層 | 【30 篇】A | 【30 篇】B | 【30 篇】C(Sonnet) | 【146 篇】A | 【146 篇】B | A 變化 | B 變化 |
|---|---|---|---|---|---|---|---|
| L1 | 2.0 | 3.5 | 3.75 | 3.0 | 4.25 | +1.0 | +0.75 |
| L2 | 4.25 | 5.0 | 4.5 | 6.0 | 6.5 | +1.75 | +1.5 |
| L3 | 4.0 | 5.75 | 6.0 | 4.25 | 6.25 | **+0.25** | +0.5 |

**tokens 對照（千）**：

| 層 | 【30 篇】A | 【30 篇】B | 【30 篇】C(Sonnet) | 【146 篇】A | 【146 篇】B | A 變化 | B 變化 |
|---|---|---|---|---|---|---|---|
| L1 | 29.5 | 33.7 | 45.0 | 31.4 | 33.6 | +6% | ±0% |
| L2 | 33.3 | 35.0 | 45.4 | 35.6 | 36.7 | +7% | +5% |
| L3 | 37.9 | **33.0** | 44.1 | 36.1 | **44.1** | **−5%** | **+33%** |

**對照分析的四個 insight**：

1. **A 組近乎規模免疫**：語料 ×4.75，tokens 只變動 −5%~+7%、L3 呼叫僅 +0.25——grep 對含獨特關鍵詞的查詢近似 O(1)（掃描毫秒級，成本在讀回內容量，而讀回量由命中數決定、不由語料大小決定）。
2. **L3 回本反轉（本輪最大發現）**：30 篇時 B 在 L3 是唯一贏面（33.0 vs 37.9，省 13%）；146 篇時同一格變成 44.1 vs 36.1（**虧 22%**）。反轉的機制不是規模本身，而是 **index 從手工 4.8K（D14 精煉）換成真實 64K（未精煉）**——導航稅隨 index 大小線性成長，一口氣吃掉並倒賠了縮小搜尋範圍的收益。
3. **L1 的變化模式印證機制**：兩組各 +0.75~1.0 次呼叫——原因相同（檔名清單／index 都變長了一頁，得多翻一次），屬於溫和的對數式成長，非爆炸。
4. **對 30 篇 C 組的推論**：C 的 30 篇數據顯示 Sonnet 的層間形狀與 Fable 一致（僅整體 +30% tokens）；已知規模效應主要作用在「讀回內容量」而非模型行為，推測 146 篇的 C 組會重現同樣形狀——留待需要時驗證，非本輪變因。

**兩項規模化定論**（已寫入規範 v1.3 第 27 條）：

1. 「50 篇建樹閾值」推測值**作廢**——146 篇實測不建樹仍最優；建樹觸發改為可觀察訊號：**grep 型查詢的呼叫數或讀回量開始隨規模顯著上漲時**才動工。
2. **導航的敵人不是規模，是肥大的 index**：建樹前提是 index 每列精煉至 D14 格式（前 60 字元識別碼）——Corpus2Skill 把導航檔壓在 <2KB/檔不是巧合，是必要條件。手工 2KB 樹（省 13%）vs 真實 64K 樹（虧 22%）的反轉就是這條規則的實驗證明。
