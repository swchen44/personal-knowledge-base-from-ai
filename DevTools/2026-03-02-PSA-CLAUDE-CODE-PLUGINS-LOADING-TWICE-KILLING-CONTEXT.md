...
| 認知層次 | 核心目的 | 對本文的具體應用 |
|---------|---------|--------------|
| **記憶（被動）** | 確認資訊存在，單純資訊檢索，確立基礎知識 | 兩個獨立臭蟲（舊版快取殘留 + 提示建構重複登錄）；修復可減少 50–60% preamble overhead；`ENABLE_TOOL_SEARCH` 旗標可延遲 MCP 工具定義載入；診斷指令（快取版本數檢查、符號連結計數、`/context` 技能表格檢查）；根本追蹤 Issue #27721 |
| **理解（半被動）** | 解釋概念的含義及關聯，串聯知識點，掌握核心邏輯 | 外掛系統的臭蟲根源在於「無去重、無清理、無作用域、無可見性」四缺。舊版快取使 Claude Code 掃描多個版本目錄，符號連結讓技能被發現兩次，加上提示建構程式碼本身再重複登錄一次，三重疊加導致每個技能可能以 3 倍以上的 token 出現在系統提示中，直接壓縮可用上下文視窗，提前觸發 Compaction。 |
| **分析（主動）** | 檢驗論點、拆解流程、找出假設，批判性思維 | 假設一：清除舊版快取能解決問題之一，但臭蟲二（提示建構）無使用者端修復方式，代表即使執行所有建議步驟仍有殘餘問題；假設二：`/context` 顯示 13% 但 `/doctor` 警告 40K tokens，代表現有監控工具存在系統性盲點，使用者無法準確掌握真實消耗；假設三：停用外掛的「disabled plugin」是否真的完全不注入 context 尚未確認，官方行為定義不明確。 |
| **應用（主動）** | 將知識套用情境，規劃執行方案 | 1. 執行舊版快取清理 Python 腳本 + 符號連結清理指令，重啟後用 `/context` 確認技能不再重複；2. 在 `settings.json` 停用當前專案不需要的外掛，並斷開未主動使用的 MCP 連接器（Gmail、GCal）；3. 設定 `enable_tool_search` 旗標延遲 MCP 工具定義載入，節省約 32K tokens。 |
| **評估（主動）** | 判斷多個方案的優劣，進行決策和權衡 | 使用者端修復（快取清理 + 符號連結清理）成本低、效果立即可見，值得立刻執行；但臭蟲二需等待 Anthropic 修復，代表這是個「部分可控」問題，對重度依賴多外掛工作流的使用者，最根本的緩解策略是減少已安裝外掛數量，或改用 `.claude/skills/` standalone 方式管理領域知識，完全繞開外掛系統的設計缺陷。 |

### 分析型追問（Socratic Follow-up）

> 以下問題供進一步反思，可用來與 AI 展開蘇格拉底式對話：

- **澄清**：「preamble overhead 減少 50–60%」是作者個人量測還是社群普遍數字？不同外掛數量與類型的使用者，實際改善幅度的差異範圍有多大？
- **假設**：文章假設 Compaction 過早觸發是這個 bug 的主要危害，但若系統提示 token 增加導致 Claude 的有效注意力分散，對回應品質的影響是否可能早於 Compaction 就已發生？
- **證據**：GitHub Issue #27721 已開放超過 4 個月且無受理人，這代表 Anthropic 認為此問題優先度低，還是有其他追蹤機制未公開？如何從外部評估一個開源 issue 的實際處理優先級？
- **觀點**：若從使用者信任的角度，一個已知 bug 存在 4 個月卻未修復的外掛系統，是否應影響使用者對外掛系統整體可靠性的評估？這對「多安裝外掛」的建議有什麼反思？
- **後果**：若 Anthropic 最終決定從根本重設計外掛系統（加入去重層），現有使用者自訂的 hook 和 MCP 設定是否有遷移成本？新架構與舊設定的相容性如何保障？
