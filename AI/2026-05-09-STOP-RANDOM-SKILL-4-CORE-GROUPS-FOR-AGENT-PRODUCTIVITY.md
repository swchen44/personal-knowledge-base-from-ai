...
- **23 個專家角色**：GStack 把 YC 創業流程裡的多角色審計搬進 Agent，一人抵一支小團隊
...
> [!tip] 實測效果
> Gary 統計，GStack 讓他的代碼產出提升了 **240 倍**，一個人能頂一支小團隊。
...
- GStack 的 23 個專家角色完整清單是什麼？每個角色的使用時機為何？（建議搜尋：GStack skill roles list）
...
| 認知層次 | 核心目的 | 對本文的具體應用 |
|---------|---------|--------------|
| **記憶（被動）** | 確認資訊存在，單純資訊檢索，確立基礎知識 | 四組 Skill 名稱：Skill creator、f skills、Superpowers、GStack、M 的 Skill、frontend design、UX Pro Max、寶玉老師 Skill 組；GStack 有 23 個專家角色；Gary 統計提升 240 倍 |
...
| **分析（主動）** | 檢驗論點、拆解流程、找出假設，批判性思維，看透策略底層邏輯 | 作者隱含的假設是「Skill 是平台無關的」，但不同 Agent（Claude Code、Cursor、Windsurf）對 Skill 格式的支援程度可能不同；GStack 的「240 倍」數據來自作者自身情境，缺乏泛化驗證 |
...
| **評估（主動）** | 判斷多個方案的優劣，進行決策和權衡，在不確定的情境中做出最佳選擇 | Superpowers 的 TDD 流程需要更長的前置時間，對於快速原型（rapid prototyping）場景可能不划算；GStack 的 23 個角色切換對新手可能門檻偏高，學習成本需納入評估 |
...
- **證據**：GStack 「240 倍提升」的測量基準是什麼？是代碼行數、任務完成時間，還是 PR merge 數量？不同指標的意義差異很大。
