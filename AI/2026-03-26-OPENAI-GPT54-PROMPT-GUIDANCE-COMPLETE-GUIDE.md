...
| 認知層次 | 核心目的 | 對本文的具體應用 |
|---------|---------|--------------|
| **記憶（被動）** | 確認資訊存在，單純資訊檢索，確立基礎知識 | 輸出契約（Output Contract）、預設執行策略（Default Follow-Through Policy）、工具持久性規則（Tool Persistence Rules）、完整性契約（Completeness Contract）、驗證循環（Verification Loop）、推理力度（Reasoning Effort：none/low/medium/high/xhigh）、研究三步驟法（Plan→Retrieve→Synthesize）、Compaction、GPT-5.4-mini 的字面性解讀、P-01 到 P-12 共 12 個 prompt 模板 |
| **理解（半被動）** | 解釋概念的含義及關聯，串聯知識點，掌握核心邏輯 | 本文的核心設計哲學是「從輸出端反向定義輸入端」：先用輸出契約明確模型應該產出什麼格式和內容，再用完整性契約定義何謂「完成」，再用驗證循環確保最終輸出的品質。推理力度被定位為「最後一哩微調」而非主要品質驅動器，這個優先順序（先完善 prompt 結構，最後才調推理力度）顛覆了大多數使用者直接調整模型參數的直覺，因為問題根源往往在 prompt 設計而非模型能力。 |
| **分析（主動）** | 檢驗論點、拆解流程、找出假設，批判性思維 | 1. 文章聲稱 GPT-5.4 在「代理人工作流程穩健性」上有顯著提升，但未提供與前版本的量化對比數據，這個主張缺乏實證支撐；2. 「工具持久性規則」要求模型「持續呼叫工具直到完成」，但沒有討論防止無限循環的機制，在邊界條件下（如搜尋結果持續為空）這個規則可能導致不必要的成本累積；3. 研究三步驟法（Plan→Retrieve→Synthesize）是合理框架，但「當繼續搜尋不太可能改變結論時停止」的停止條件依賴模型的主觀判斷，缺乏可量化的終止標準。 |
| **應用（主動）** | 將知識套用情境，規劃執行方案 | 1. 立即將 P-01（輸出契約）套用至日常報告生成任務：明確列出每個輸出區塊的格式和字數限制，消除模型自由發揮帶來的格式不一致；2. 在代理人 system prompt 中加入 P-02（預設執行策略），定義「可逆動作直接執行、不可逆動作詢問確認」的行為邊界，減少不必要的中斷詢問；3. 使用 P-06（研究三步驟法）框架改寫現有的研究型 prompt，從「幫我研究 X」改為「用三步驟法研究 X：先列子問題、再逐一搜尋、最後合成附引用」。 |
| **評估（主動）** | 判斷多個方案的優劣，進行決策和權衡 | 本文的 12 個 prompt 模板覆蓋了從輸出控制到代理人行為的完整光譜，相較於「一個萬能 system prompt」的做法，模組化模板的優勢是可以按場景組合，缺點是需要使用者有足夠的判斷力選擇合適的組合。推理力度的分級（none→xhigh）提供了成本與品質的旋鈕，但「xhigh 保留給長時間、代理人式、推理密集的任務」這個建議過於模糊，缺乏具體的任務類型判斷標準。對比 Anthropic 的 Claude prompt 最佳實踐，GPT-5.4 的指南更強調「輸出契約」和「完整性契約」這兩個概念，顯示 OpenAI 針對可靠執行（reliable execution）的設計取向有別於強調安全性和憲法 AI（Constitutional AI）的 Anthropic。 |

### 分析型追問（Socratic Follow-up）

> 以下問題供進一步反思，可用來與 AI 展開蘇格拉底式對話：

- **澄清**：「輸出契約（Output Contract）」要求模型「按指定順序回傳確切的請求區塊」，但若模型本身無法完成某個區塊（如因資料不足），正確的行為應該是標記缺失還是略過該區塊？這個邊界情況本文沒有說明。
- **假設**：「先完善 prompt 結構，最後才調推理力度」這個建議假設 prompt 結構問題比模型能力問題更常見。這個假設適用於所有任務類型嗎？對於高度需要推理的數學或邏輯問題，這個順序是否仍然成立？
- **證據**：「GPT-5.4-mini 的更字面性解讀」是一個行為觀察，但沒有說明這個特性在什麼樣的測試集上被系統性驗證。這個觀察是否只在英文場景成立，中文提示詞是否表現不同？
- **觀點**：從使用者自主性的角度，「驗證循環（Verification Loop）」要求模型在執行「有外部影響的動作前確認安全性」——但「安全性」由誰定義？若模型對安全的判斷與使用者不同，這個機制是否反而製造了摩擦？
- **後果**：若 P-03（工具持久性）被廣泛採用到所有代理人 system prompt，而模型在某些場景下確實進入「持續工具呼叫」的循環，組織的 API 成本控制機制是否能及時察覺並介入？預算超支的風險管理應如何設計？
