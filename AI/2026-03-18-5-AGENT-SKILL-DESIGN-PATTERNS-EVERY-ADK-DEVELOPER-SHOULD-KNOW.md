...
| 認知層次 | 核心目的 | 對本文的具體應用 |
|---------|---------|--------------|
| **記憶（被動）** | 確認資訊存在，單純資訊檢索，確立基礎知識 | 5 種設計模式（工具包裝器 Tool Wrapper、生成器 Generator、審查器 Reviewer、反轉模式 Inversion、管線 Pipeline）、SKILL.md 格式、ADK（Agent Development Kit）、`references/` 目錄、`assets/` 目錄、漸進式揭露（Progressive Disclosure）、`DO NOT start building until all phases are complete`、SkillToolset、上下文視窗（Context Window）、決策樹 |
| **理解（半被動）** | 解釋概念的含義及關聯，串聯知識點，掌握核心邏輯 | 五種模式的核心邏輯共同指向一個原則：將「何時做」與「如何做」分離。Tool Wrapper 讓代理人按需取得框架知識；Generator 把輸出結構外部化到模板；Reviewer 把審查標準外部化到清單；Inversion 翻轉「先猜後做」為「先問後做」；Pipeline 強制執行嚴格順序。這五種模式實際上是在回應「代理人最常出現哪些行為偏差」——分別是知識污染、輸出不一致、評審標準模糊、過早假設、跳步執行。 |
| **分析（主動）** | 檢驗論點、拆解流程、找出假設，批判性思維 | 1. 文章聲稱 30+ 個代理人工具都採用相同 SKILL.md 格式，但未說明這是自然收斂還是有標準化組織推動，這個「統一」的穩定性值得懷疑；2. 反轉模式（Inversion）的「DO NOT start building」依賴模型的指令遵從性，但在上下文接近上限時，模型跳過約束的傾向是否已被系統性測試？；3. 五種模式被呈現為互補而非競爭關係，但文章未說明某些任務類型若強行套入特定模式反而降低品質的反例。 |
| **應用（主動）** | 將知識套用情境，規劃執行方案 | 1. 將現有的 SKILL.md 對照決策樹檢視：它屬於哪種模式？若橫跨多種模式則考慮拆分；2. 對現有的程式碼審查 skill，將審查標準（rubric）抽離到 `references/review-checklist.md`，讓切換場景（如從 Python 風格審查切換到 OWASP 安全審查）只需替換一個檔案；3. 設計新的需求收集 skill 時，使用 Inversion 模式明確設計 Phase 1-3 的問題序列與閘道條件，防止代理人過早生成設計文件。 |
| **評估（主動）** | 判斷多個方案的優劣，進行決策和權衡 | 相較於「把所有指令寫在一個龐大的 SKILL.md」，模式化設計的優勢是可維護性和可組合性，但代價是設計複雜度更高且需要熟悉模式語彙。Pipeline 模式提供最強的執行保證，但最不靈活；Tool Wrapper 最靈活，但對代理人自主判斷依賴最高。在不允許出錯的場景（如 DB 遷移、部署），應優先選擇 Pipeline；在探索性任務（如研究、分析），Tool Wrapper + Reviewer 的組合提供更好的適應性。模式的最大價值不在個別使用，而在組合：Generator + Inversion 或 Pipeline + Reviewer 能有效補足各自的弱點。 |

### 分析型追問（Socratic Follow-up）

> 以下問題供進一步反思，可用來與 AI 展開蘇格拉底式對話：

- **澄清**：「漸進式揭露（Progressive Disclosure）」在 SKILL.md 中，是指令代理人「在需要時去讀取某個檔案」——但「需要」的時機判斷是由代理人自主決定，還是由指令明確規定？這兩種方式的可靠性差異有多大？
- **假設**：文章假設五種模式能覆蓋「絕大多數工作流程」，但這個假設是基於什麼樣的工作流程樣本？是否有明顯的工作流程類型落在這五種模式之外？
- **證據**：「模式可以組合（patterns compose）」是直覺上合理的主張，但在實際測試中，組合後的 skill 是否確實比單一模式更可靠，還是複雜度增加反而帶來不穩定性？
- **觀點**：從以人為本的設計角度，反轉模式（Inversion）要求代理人反覆提問才能開始執行，在使用者時間緊迫時，這種「強迫問答」模式是否反而降低使用體驗？
- **後果**：若 SKILL.md 格式成為跨工具的事實標準，不同工具對相同 skill 的解讀是否會出現不一致？這種「假統一」可能帶來哪些預期之外的相容性問題？
